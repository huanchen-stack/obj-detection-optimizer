...
def draw(config):
    ...
    df_4GB['energy'] += base
    plot_data = df_4GB['energy']/1000
    for i in range(len(plot_data)):
        plot_data[i] = plot_data[i] * ((1-df_4GB['optimizer'][i]*0.01) * benchmarks['1'][config])
    ...
    swap_4G_USB = (base + swap_power[config] * swap_mem_baseline['4GB'][config] / 1280) * (benchmarks['1'][config] + swap_mem_baseline['4GB'][config] / 1280)
    l1 = ax1.axhline(y=swap_4G_USB/1000, color='b', linestyle='--', label='Baseline (USB)')
    swap_4G_MicroSD = (base + swap_power[config] * swap_mem_baseline['4GB'][config] / 190) * (benchmarks['1'][config] + swap_mem_baseline['4GB'][config] / 190)
    l3 = ax1.axhline(y=swap_4G_MicroSD/1000, color='b', linestyle='-', label='Baseline (MicroSD)')

    b1 = ax2.bar(x1_list, plot_data, width=w, label='SN', color=sns.xkcd_rgb["maize"])
    swap_4G_USB = (base + swap_power[config] * swap_mem_baseline['4GB'][config] / 1280) * (benchmarks['1'][config] + swap_mem_baseline['4GB'][config] / 1280)
    l1 = ax2.axhline(y=swap_4G_USB/1000, color='b', linestyle='--', label='Baseline (USB)')
    swap_4G_MicroSD = (base + swap_power[config] * swap_mem_baseline['4GB'][config] / 190) * (benchmarks['1'][config] + swap_mem_baseline['4GB'][config] / 190)
    l3 = ax2.axhline(y=swap_4G_MicroSD/1000, color='b', linestyle='-', label='Baseline (MicroSD)')
    ...
    # plt.grid()
    plt.savefig(f"EDP/{config}-4GB.png", bbox_inches='tight', dpi=100)
    plt.savefig(f"EDP/pdfs/{config}-4GB.pdf", bbox_inches='tight', dpi=100)
