...
class OPT_WRAPPER_MULTIDEVICE(object):
    ...
    def optimize_once(self, bandwidth, prof_list, reverse0, reverse1, iterations=None):
        """
    ...
    def optimize(self):
        ...
        for bandwidth in self.bandwidth_list:
            ...
            for num_devices in range(1, self.num_devices_max + 1):
                # Generate prof_list of num_devices devices (combinations):
                prof_lists = set(itertools.combinations(self.prof, num_devices))
                for prof_list in prof_lists:
                    # opt_wrapper iterates through all possible number of devices and pick
                    #   the most optimal one (usually the more devices you use, the more speedup
                    #   you can get, but the more extra energy consumption would be incurred):

                    # four different heuristics are tried, we pick the best of all heuristics
                    opt1 = self.optimize_once(bandwidth, prof_list, True, True)
                    opt2 = self.optimize_once(bandwidth, prof_list, True, False)
                    opt3 = self.optimize_once(bandwidth, prof_list, False, True)
                    opt4 = self.optimize_once(bandwidth, prof_list, False, False)
            ...
            self.opt_num_devices.append(len(best[-1]))
            self.opt_speedup_rate.append(best[1])
            for i in range(len(across_devices)):
                num_devices = len(across_devices[i][5])
                # if (across_devices[i][0] - best[0]) / best[0] <= 1 - self.threshold:
                if across_devices[i][0] <= best[0] * self.threshold:
            ...
            args = [bandwidth, best[-1], best[3], best[4], best[2]]
    ...
    def report(self):
        return {
            ...
            'opt_speedup_rate': [(1 - r/100) * self.benchmark for r in self.opt_speedup_rate],
...
def driver(config, threshold):
    ...
    devices = [
        ...
        ['agx', 'nano', 'nano', 'nano', 'nano', 'nano', 'nano', 'nano'],
        ['agx', 'agx', 'nano', 'nano', 'nano', 'nano', 'nano', 'nano'],
        ['agx', 'agx', 'agx', 'nano', 'nano', 'nano', 'nano', 'nano'],
        ['agx', 'agx', 'agx', 'agx', 'nano', 'nano', 'nano', 'nano', 'nano'],
        ['agx', 'agx', 'agx', 'agx', 'agx', 'agx', 'agx', 'agx', 'agx'],
        # ['agx', 'nano', 'nano', 'nano', 'nano', 'nano', 'nano', 'nano', 'agx'],
        # ['agx', 'nano', 'agx'],
        # ['agx', 'agx'],
        # ['agx', 'agx'],
    ]
    ...
    for device_list in devices:
        ...
        res_list.append( f"{res['bandwidths'][0]},{res['opt_speedup_rate'][0]},0,{res['opt_num_devices'][0]},{res['payload'][0]}\n")
